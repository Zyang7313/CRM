...
<script type="text/javascript">
	$(function(){
		...
		//为 选择（市场活动）按钮绑定事件，填充市场活动源（并将选择的市场活动id赋值到隐藏域中）
		$("#selectedActivityBtn").click(function () {
		...
		$("#convertBtn").click(function () {
			...
			*/
			if($("#isCreateTransaction").prop("checked")){
				...
				//alert("需要创建交易")
				//需要创建交易时，需要的参数有：
				// clueId，交易金额money，交易名称name，预计成交日期expectedDate，阶段stage，activityId
				...
				//以上传递参数的方式不适用与参数、内容过多的情况，并且一旦参数扩充，挂载的参数有可能超过地址栏的上限
				//我们想到使用提交表单的方式来发出本次的传统请求，并且提交表单能够使用post请求
				$("#tranForm").submit();

			}else{

				//alert("不需要创建交易")
				//不需要创建交易的时候只需要一个参数，clueId
	...
	function showModalActivityList(){

		$.ajax({

			url : "workbench/clue/getActivityListByName.do",
			data : {
				"aname":$.trim($("#search-aname").val())
			},
			type : "get",
			dataType : "json",
		...
		<div class="modal-dialog" role="document" style="width: 90%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="selectedActivityBtn">选择</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form id="tranForm" action="workbench/clue/convert.do" method="post">
		  ...
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    ...
		    <input type="text" class="form-control time" id="expectedClosingDate" name="expectedName">
		  </div>
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
